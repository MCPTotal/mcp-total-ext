    ...
    window.fetch = async function() {
      ...
      if (typeof url === 'string' && url.includes('/backend-api/conversation')) {
        ...
        if (contentType.includes('text/event-stream')) {
          ...
          self.processStreamForToolCalls(clonedResponse.body, writable).then(toolCalls => {
        ...
        if (done) {
          ...
          if (toolCalls.length > 0) {
            ...
            return toolCalls.map(toolCall => {
              ...
              toolCall.execute = (params) => boundExecuteToolCall(toolCall.tool, params || toolCall.parameters);
              return toolCall;
            });
          }
          break;
